...
@section('content')
    <header class="header">
        <form class="search-form" action="{{ route('dashboard.products.search') }}" method="get">
            @csrf
            <input class="search-input" type="search" name="keyword" placeholder="Поиск по Продуктам..." autocomplete="off"
        ...
        <ul class="header-navigation">
            ...
            <li class="header-navigation-item">
                <a class="header-navigation-link" href="{{ route('dashboard.products.categories') }}">Категории:
                    {{ $categories->count() }}</a>
            </li>
            <li class="header-navigation-item">
                <a class="header-navigation-link green-bg white" href="{{ route('dashboard.products.create') }}">Добавить
                    новый продукт</a>
            </li>
        </ul>
    </header>
    <ul class="breadcrumbs book-read-page__breadcrumbs">
        ...
        <li class="breadcrumbs-item">
            <a class="breadcrumbs-link current"
                href="{{ route('dashboard.products.update', $product->id) }}">{{ $product->ru_title }}</a>
        </li>
    </ul>
    <main class="products-update-page">
        <form class="form" action="{{ route('products.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <fieldset class="form__element form__element--long form__element--photo">
                ...
                <img class="form__photo-preview {{ $product->img ? 'transparent-bg' : '' }}"
                    src="{{ asset('img/products/' . $product->img) }}" alt="Фотография продукта" width="40"
                    height="44">
            ...
            <fieldset class="form__element form__element--long">
                ...
                <input class="product-icon-input visually-hidden" id="gel" type="radio" name="icon"
                    value="gel.svg" {{ $product->icon === 'gel.svg' ? 'checked' : '' }}>
                <label class="product-icon product-icon--gel" for="gel">Гель</label>
                <input class="product-icon-input visually-hidden" id="draje" type="radio" name="icon"
                    value="draje.svg" {{ $product->icon === 'draje.svg' ? 'checked' : '' }}>
                <label class="product-icon product-icon--draje" for="draje">Драже</label>
                <input class="product-icon-input visually-hidden" id="drops" type="radio" name="icon"
                    value="drops.svg" {{ $product->icon === 'drops.svg' ? 'checked' : '' }}>
                <label class="product-icon product-icon--drops" for="drops">Капли</label>
                <input class="product-icon-input visually-hidden" id="capsules" type="radio" name="icon"
                    value="capsules.svg" {{ $product->icon === 'capsules.svg' ? 'checked' : '' }}>
                <label class="product-icon product-icon--capsules" for="capsules">Капсулы</label>
                <input class="product-icon-input visually-hidden" id="cream" type="radio" name="icon"
                    value="cream.svg" {{ $product->icon === 'cream.svg' ? 'checked' : '' }}>
                <label class="product-icon product-icon--cream" for="cream">Крем</label>
                <input class="product-icon-input visually-hidden" id="mix" type="radio" name="icon"
                    value="mix.svg" {{ $product->icon === 'mix.svg' ? 'checked' : '' }}>
                <label class="product-icon product-icon--mix" for="mix">Раствор</label>
                <input class="product-icon-input visually-hidden" id="syrop" type="radio" name="icon"
                    value="syrop.svg" {{ $product->icon === 'syrop.svg' ? 'checked' : '' }}>
                <label class="product-icon product-icon--syrop" for="syrop">Сироп</label>
                <input class="product-icon-input visually-hidden" id="sprey" type="radio" name="icon"
                    value="sprey.svg" {{ $product->icon === 'sprey.svg' ? 'checked' : '' }}>
                <label class="product-icon product-icon--sprey" for="sprey">Спрей</label>
                <input class="product-icon-input visually-hidden" id="suspension" type="radio" name="icon"
                    value="suspension.svg" {{ $product->icon === 'suspension.svg' ? 'checked' : '' }}>
                <label class="product-icon product-icon--suspension" for="suspension">Суспензия</label>
                <input class="product-icon-input visually-hidden" id="tablets" type="radio" name="icon"
                    value="tablets.svg" {{ $product->icon === 'tablets.svg' ? 'checked' : '' }}>
                <label class="product-icon product-icon--tablets" for="tablets">Таблетки</label>
                <input class="product-icon-input visually-hidden" id="injuction" type="radio" name="icon"
                    value="injuction.svg" {{ $product->icon === 'injuction.svg' ? 'checked' : '' }}>
                <label class="product-icon product-icon--injuction" for="injuction">Укол</label>
                <input class="product-icon-input visually-hidden" id="ampulse" type="radio" name="icon"
                    value="ampulse.svg" {{ $product->icon === 'ampulse.svg' ? 'checked' : '' }}>
            ...
            <fieldset class="form__element">
                ...
                <input class="form__input" type="text" name="ru-title" id="ru-title" placeholder="Лактоспей беби"
                    autocomplete="off" required data-pristine-required-message="Объязательное поле"
                    value="{{ $product->ru_title }}">
            </fieldset>
            <fieldset class="form__element">
                ...
                <input class="form__input" type="text" name="en-title" id="en-title" autocomplete="off"
                    placeholder="Lactospey Baby" required data-pristine-required-message="Объязательное поле"
                    value="{{ $product->en_title }}">
            </fieldset>
            <fieldset class="form__element">
                <label class="form__label" for="mn-title">Название на монгольском</label>
                <input class="form__input" type="text" name="mn-title" id="mn-title" autocomplete="off"
                    placeholder="Lactospey Baby" required data-pristine-required-message="Объязательное поле"
                    value="{{ $product->mn_title }}">
            </fieldset>
            <div></div>
            <fieldset class="form__element form__element--file">
                <legend class="form__label">Инструкция на русском</legend>
                <p class="form__file-preview" data-instruction="ru">
                    {{ $product->ru_instruction ? $product->ru_instruction : 'Файл не выбран' }}</p>
                <div class="form-actions__wrap">
                    ...
                    <button class="form-actions__delete" data-action="delete-ru-instruction"
            ...
            <fieldset class="form__element form__element--file">
                ...
                <p class="form__file-preview" data-instruction="en">
                    {{ $product->en_instruction ? $product->en_instruction : 'Файл не выбран' }}</p>
                <div class="form-actions__wrap">
                    ...
                    <button class="form-actions__delete" data-action="delete-en-instruction"
                        type="button">Удалить</button>
                    <input class="visually-hidden" type="checkbox" name="en-instruction-deleted">
                </div>
            </fieldset>
            <fieldset class="form__element form__element--file">
                <legend class="form__label">Инструкция на монгольском</legend>
                <p class="form__file-preview" data-instruction="mn">
                    {{ $product->mn_instruction ? $product->mn_instruction : 'Файл не выбран' }}</p>
                <div class="form-actions__wrap">
                    <label class="form-actions__edit" for="mn-instruction">Редактировать</label>
                    <input class="visually-hidden" id="mn-instruction" type="file" name="mn-instruction">
                    <button class="form-actions__delete" data-action="delete-mn-instruction"
                        type="button">Удалить</button>
                    <input class="visually-hidden" type="checkbox" name="mn-instruction-deleted">
                </div>
            </fieldset>
            ...
            <fieldset class="form__element form__element--wide form__element--desc">
                <legend class="form__label">Описание на монгольском</legend>
                <textarea class="form__textarea" name="mn-description" placeholder="Англи хэл дээрх бүтээгдэхүүний тайлбар">{{ $product->mn_description }}</textarea>
            </fieldset>
            ...
            <fieldset class="form__element form__element--wide form__element--long">
                <legend class="form__label">Состав на монгольском</legend>
                <textarea class="simditor-textarea" name="mn-composition" placeholder="Англи хэл дээрх бүтээгдэхүүний найрлага">{{ $product->mn_composition }}</textarea>
            </fieldset>
            <fieldset class="form__element form__element--wide form__element--long">
            ...
            <fieldset class="form__element form__element--wide form__element--long">
                <legend class="form__label">Показания к применению на монгольском</legend>
                <textarea class="simditor-textarea" name="mn-indications" placeholder="Англи хэл дээр хэрэглэх заалт">{{ $product->mn_indications }}</textarea>
            </fieldset>
            <fieldset class="form__element form__element--wide form__element--long">
            ...
            <fieldset class="form__element form__element--wide form__element--long">
                <legend class="form__label">Способ применения на монгольском</legend>
                <textarea class="simditor-textarea" name="mn-method" placeholder="Англи хэл дээрх аргыг хэрхэн ашиглах вэ">{{ $product->mn_method }}</textarea>
            </fieldset>
            <fieldset class="form__element form__element--submit">
                <button class="form__submit" type="submit">Редактировать</button> или <a class="form__reset"
                    href="javascript:window.location.href=window.location.href">отменить</a>
            </fieldset>
        </form>
    </main>
@endsection

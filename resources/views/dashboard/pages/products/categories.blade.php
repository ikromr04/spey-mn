...
@section('content')
    <header class="header">
        <form class="search-form" action="{{ route('dashboard.products.categories.search') }}" method="get">
            @csrf
            <input class="search-input" type="search" name="keyword" placeholder="Поиск по Категориям..." autocomplete="off"
        ...
        <ul class="header-navigation">
            ...
            <li class="header-navigation-item">
                <a class="header-navigation-link current" href="{{ route('dashboard.products.categories') }}">Категории:
                    {{ $categoriesQuantity }}</a>
            </li>
            <li class="header-navigation-item">
                <button class="header-navigation-link green-bg white" type="button" data-action="create-category">Добавить
                    новую категорию</button>
            </li>
        </ul>
    </header>
    ...
    <main class="products-categories-page">
        ...
        <div class="confirm-modal hidden">
            <form class="confirm-form" action="{{ route('products.categories.delete') }}" method="post">
                @csrf
        ...
        <div class="create-modal hidden">
            <form class="create-form" action="{{ route('products.categories.create') }}" method="post">
                @csrf
        ...
        <div class="update-modal hidden">
            <form class="update-form" action="{{ route('products.categories.update') }}" method="post">
                @csrf
                <input type="hidden" name="id" value="">
                <label class="confirm-label">Название категории на русском
